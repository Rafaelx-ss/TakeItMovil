...
export default function CrearUsuario() {
  ...
  const editarUsuario = async () => {
    try {
      await UsersService.editarUsuario(Number(usuarioID), nombreUsuario, usuario, email, password, telefonoUsuario, generoUsuario, rolUsuario);
  ...
  useEffect(() => {
    const fetchUsuario = async () => {
      if (usuarioID) {
        const usuario = await UsersService.getUsuario(Number(usuarioID));
        setNombreUsuario(usuario.nombreUsuario);
        setUsuario(usuario.usuario);
        setEmail(usuario.email);
        setPassword(usuario.password);
        setTelefonoUsuario(usuario.telefonoUsuario);
        setGeneroUsuario(usuario.generoUsuario);
        setRolUsuario(usuario.rolUsuario);
      }
    };
    fetchUsuario();
  }, []);
  ...
  return (
    <ScrollView contentContainerStyle={{ flexGrow: 1 }}>
      ...
      <View className="flex-1 bg-background">
        {/* Encabezado */}
        <HeaderGradient title="Edita el usuario" showBackButton onBackPress={() => router.back()} />
        ...
        <View className='p-5 rounded-lg mb-4 shadow-md'>
          ...
          <Button title="Crear Usuario" onPress={editarUsuario} />
        </View>
      </View>
    </ScrollView>
  );
